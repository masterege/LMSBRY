...
def profile(request):
    ...
    if request.user.is_lecturer:
        courses = Course.objects.filter(allocated_course_lecturer_pk = request.user.id).filter(
        ...
        level = Student.objects.get(student_pk = request.user.id)
        try:
            parent = Parent.objects.get(student = level)
        except:
            parent = "no parent set"
        courses = TakenCourse.objects.filter(student__student_id = request.user.id, course_level = level.level)
        context = {
            'title': request.user.get_full_name,
            'parent': parent,
            'courses': courses,
...
def profile_single(request, id):
    ...
    if user.is_lecturer:
        courses = Course.objects.filter(allocated_course_lecturer_pk = id).filter(semester = current_semester)
        context = {
        ...
        student = Student.objects.get(student_pk = id)
        courses = TakenCourse.objects.filter(student_student_id = id, course_level = student.level)
        context = {
            'title': user.get_full_name,
            'user': user,
            "user_type": "student",
