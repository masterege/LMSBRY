...
#             or_lookup = (Q(username_icontains = query) | 
...
class StudentManager(models.Manager):
    def search(self, query = None):
        qs = self.get_queryset()
        if query is not None:
            or_lookup = (Q(level_icontains=query) | 
                         Q(department_icontains=query)
                        )
            qs = qs.filter(or_lookup).distinct() # distinct() is often necessary with Q lookups
        return qs
...
class Parent(models.Model):
    ...
    # What is the relationship between the student and the parent (i.e. father, mother, other)
    relation_ship = models.TextField(choices = relationship, blank = True)
