...
def program_view(request):
    ...
    if program_filter:
        programs = Program.objects.filter(title_icontains = program_filter)
...
def course_single(request, slug):
    ...
    files = Upload.objects.filter(course_slug = slug)
    videos = UploadVideo.objects.filter(course_slug = slug)

    lecturers = CourseAllocation.objects.filter(courses_pk = course.id)
...
def course_registration(request):
    if request.method == 'POST':
        ...
        for s in range(0, len(ids)):
            student = Student.objects.get(student_pk = request.user.id)
        ...
        student = get_object_or_404(Student, student_id = request.user.id)
        taken_courses = TakenCourse.objects.filter(student_student_id = request.user.id)
        t = ()
        for i in taken_courses:
            t += (i.course.pk,)
        current_semester = Semester.objects.get(is_current_semester = True)

        courses = Course.objects.filter(program_pk=student.department.id, level = student.level, semester = current_semester
        ).exclude(id_in = t).order_by('year')
        all_courses = Course.objects.filter(level = student.level, program_pk = student.department.id)

        no_course_is_registered = False  # Check if no course is registered
        all_courses_are_registered = False

        registered_courses = Course.objects.filter(level=student.level).filter(id_in = t)
...
def user_course_list(request):
    if request.user.is_lecturer:
        courses = Course.objects.filter(allocated_course_lecturer_pk = request.user.id)

        return render(request, 'course/user_course_list.html', {'courses': courses})

    elif request.user.is_student:
        student = Student.objects.get(student_pk = request.user.id)
        taken_courses = TakenCourse.objects.filter(student_student_id = student.student.id)
        courses = Course.objects.filter(level=student.level).filter(program_pk = student.department.id)
